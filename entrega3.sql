...
BEGIN
	...
	SELECT @dniPaciente = numDoc FROM dbHospital.paciente
	WHERE numDoc = @dni
...
CREATE TABLE dbHospital.domicilio(
	...
	CONSTRAINT ck_cpDomicilio CHECK (LEN(codigoPostal) = 4 ) -- el cod postal debe tener longitud = 4
);
GO
...
CREATE TABLE dbHospital.prestador (
	...
	activo				bit default 1, --para saber si el contrato sigue o no vigente, por default decimos que si
	id_cobertura		int,
...
CREATE TABLE dbHospital.diasXsede(
	id_dia_sede			int IDENTITY (1,1) PRIMARY KEY,
